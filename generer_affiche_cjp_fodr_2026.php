...
// ========== FONCTION POUR COMPRESSER ET REDIMENSIONNER L'IMAGE (PRIORITE HAUT) ==========
function compressAndResizeImage($source_path, $max_width = 500, $max_height = 500, $quality = 75) {
    ...
    // Créer une nouvelle image carrée de la taille cible
    ...
    if ($type == IMAGETYPE_PNG) {
        ...
        imagefilledrectangle($destination, 0, 0, $max_width, $max_height, $transparent);
    } else {
        // Fond blanc pour JPEG
        $white = imagecolorallocate($destination, 255, 255, 255);
        imagefilledrectangle($destination, 0, 0, $max_width, $max_height, $white);
    }
    
    // Calculer le ratio pour remplir le cadre
    $ratio_w = $max_width / $width;
    $ratio_h = $max_height / $height;
    
    // On prend le plus grand ratio pour couvrir toute la zone
    $ratio = max($ratio_w, $ratio_h);
    
    $new_width = round($width * $ratio);
    $new_height = round($height * $ratio);
    
    // PRIORITÉ AU HAUT : y = 0 (collé en haut, pas centré)
    // Pour la largeur, on centre
    $x = ($max_width - $new_width) / 2;
    $y = 0;  // ← IMPORTANT: Collé en haut, pas centré verticalement
    
    // Redimensionner et recadrer (priorité haut)
...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    ...
    <body class="gradient-bg min-h-screen flex items-center justify-center p-6">
        <div class="max-w-2xl w-full bg-white rounded-3xl shadow-2xl overflow-hidden">
            ...
            <form action="" method="POST" enctype="multipart/form-data" class="p-8 space-y-5" id="participantForm">
                ...
                <!-- Champs pour Paneliste (theme + poste) -->
                <div id="themeField" class="hidden-field">
                    ...
                    <!-- NOUVEAU CHAMP : Poste / Service du paneliste -->
                    <label class="block text-sm font-semibold text-gray-700 mb-2 mt-4">Poste / Service *</label>
                    <input type="text" name="poste_paneliste" class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-[#008878] focus:outline-none transition" placeholder="Ex: Directeur IT, Consultant, Expert Cybersécurité, etc.">
                </div>
    ...
    <script>
        ...
        function updateFields() {
            ...
            document.querySelector('input[name="poste_paneliste"]').disabled = true;
            ...
            if (statut === 'Formateur') {
                ...
                document.querySelector('input[name="poste_paneliste"]').disabled = false;
                document.querySelector('input[name="poste_paneliste"]').required = true;
            } else {
                ...
                document.querySelector('input[name="poste_paneliste"]').required = false;
            }
        }
...
$poste_paneliste = htmlspecialchars(trim($_POST['poste_paneliste'] ?? '')); // NOUVEAU CHAMP
...
if ($statut === 'Paneliste') {
    ...
    if (empty($poste_paneliste)) {
        die("<div class='bg-red-100 text-red-700 p-4 rounded-lg text-center'>❌ Erreur: Veuillez saisir votre poste ou service. <a href='javascript:history.back()'>Retour</a></div>");
    }
}

// ========== VÉRIFICATION DE LA LIMITE DES AFFICHES ==========
try {
            ...
            <div class="modal-overlay">
                <div class="modal-container">
                    ...
                    <div class="modal-limit-badge">
                        <span>⚠️ Affiches disponibles</span>
                    </div>
                    <p class="modal-message">
                        <strong><?php echo htmlspecialchars($nom); ?></strong>, vous avez déjà créé <strong style="color:#ff6b6b"><?php echo $afficheCount; ?> affiche(s)</strong>.
                    </p>
                    <div>
...
// Modification de la table pour ajouter le champ poste_paneliste
try {
    $pdo->exec("ALTER TABLE participants ADD COLUMN IF NOT EXISTS poste_paneliste VARCHAR(255)");
} catch(PDOException $e) {
...
try {
    $stmt = $pdo->prepare("INSERT INTO participants (nom, statut, titre_formation, theme_panel, poste_paneliste, photo_base64, fonction_affiche, code_unique, code_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->execute([$nom, $statut, $titre_formation, $theme_panel, $poste_paneliste, $photo_base64, $fonction_affiche, $code_unique, $code_id]);
    $participant_id = $pdo->lastInsertId();
} catch(PDOException $e) {
    die("<div class='bg-red-100 text-red-700 p-4 rounded-lg text-center'>❌ Erreur lors de l'enregistrement : " . $e->getMessage() . "</div>");
}

$remaining = 100 - ($afficheCount + 1);
$limitMessage = ($remaining <= 0) ? "<p class='text-orange-600 text-1xl mt-2'>⚠️ Limite approchée.</p>" : "<p class='text-green-600 text-sm mt-2'>✓ Inscription enregistrée</p>";
?>
    ...
    <div id="affiche" class="w-[800px] shadow-2xl overflow-hidden" style="background: linear-gradient(45deg, #92d7ff 0%, #fffcfc 50%, #92d7ff 100%);">
        ...
        <div class="p-4">
            <div class="flex justify-center">
                <div class="mt-4">
                    <img src="img/logo.png" alt="cjp" class="w-28 rounded-xl mt-0">
                </div>
                <div class="text-center"> <br>
                    <p class="text-black text-2xl mt-1 font-black text-black">FORUM DU DÉVELOPPEMENT WEB <br> ET RÉSEAUX</p>
                    <p>Panel, Formations, Hackathon et Présentation de projets</p>
                    <span class="text-yellow-500 text-2xl">4ÈME ÉDITION</span>                      
                </div>  
                 <div class="mt-4">
                    <img src="img/Logo_univ_labe.png" alt="Logo-CJP" class="w-48 rounded-xl mt-5">
                </div>
            </div>
        </div>
        <hr class="border-1xl">
        ...
        <div class="p-8">
            <div class="flex flex-col items-center">
                <div class="flex items-end justify-center gap-6">
                    ...
                    <div class="relative px-8 -top-20 py-6 text-center min-w-[180px] transform rotate-1 transition ml-20 mb-20">
                        ...
                        <div class="text-yellow-500 font-black text-5xl leading-tight tracking-tighter">J'Y<br>SERAI !</div>
                    </div>
                </div>
                    ...
                    <div class="text-left relative -ml-15 w-64 flex-shrink-0">
                        <div class="w-full">
                           <h6 class="text-2xl text-center font-black text-gray-800 break-words whitespace-normal"><?php echo strtoupper($nom); ?></h6>
                                ...
                                <!-- AFFICHAGE DU POSTE POUR PANELISTE (juste en dessous du nom) -->
                                <?php if ($statut === 'Paneliste' && !empty($poste_paneliste)): ?>
                                    <div class="flex justify-center mt-1">
                                        <p class="text-base font-semibold text-gray-600 text-center"><?php echo $poste_paneliste; ?></p>
                                    </div>
                                <?php endif; ?>
            ...
            <div class="flex justify-between items-center border-b border-yellow-400 pb-1 mb-3">
                <p class="text-1xl font-bold text-black uppercase tracking-wider">Nos partenaires</p>
            </div>
            ...
            <div class="flex items-center justify-center gap-2 mt-4 flex-wrap">
                ...
                <img src="img/Aguipe2.png" alt="" class="w-20 rounded-xl p-2">
                ...
                <img src="img/motech.jpeg" alt="" class="w-20 rounded-xl p-2">
                <img src="img/KARKI.jpg" alt="" class="w-20 rounded-xl p-2">   
                ...
                <img src="img/groupemoubarack.jpeg" alt="" class="w-20 rounded-xl p-2"> 
                <img src="img/DOZTRA.jpeg" alt="" class="w-20 rounded-xl p-2"> 
                <img src="img/village.jpeg" alt="" class="w-20 rounded-xl p-2"> 
                <img src="img/csig.jpeg" alt="" class="w-20 rounded-xl p-2"> 
                <img src="img/dba.jpeg" alt="" class="w-20 rounded-xl p-2"> 
                <img src="img/fondation_afrique_emergente.jpeg" alt="" class="w-20 rounded-xl p-2"> 
                <img src="img/jaabi.jpeg" alt="" class="w-20 rounded-xl p-2"> 
                <img src="img/netsengroup.png" alt="" class="w-20 rounded-xl p-2"> 
                <img src="img/keevent.png" alt="" class="w-20 rounded-xl p-2"> 
                <img src="img/nimba.jpeg" alt="" class="w-20 rounded-xl p-2"> 
                <img src="img/Afrinov.jpeg" alt="" class="w-20 rounded-xl p-2">         
                <img src="img/oa.jpeg" alt="" class="w-20 rounded-xl p-2"> 
                <img src="img/russie.jpeg" alt="" class="w-20 rounded-xl p-2"> 
                <img src="img/soutra.png" alt="" class="w-20 rounded-xl p-2">
                <img src="img/talentgn.jpeg" alt="" class="w-20 rounded-xl p-2">
                <img src="img/titan_Deploy.jpeg" alt="" class="w-20 rounded-xl p-2"> 
                <img src="img/ADA.png" alt="" class="w-20 rounded-xl p-2"> 
                <img src="img/arclytics.jpeg" alt="" class="w-20 rounded-xl p-2"> 
            </div>
        </div>
        ...
        <div class="p-6 text-center" style="background: linear-gradient(45deg, #92d7ff 0%, #92d7ff 50%, #ffffff 100%);">
            <div class="flex justify-between items-center">
                <div class="text-left">
                    <p class="text-black text-2xl">www.club-jp.com</p>
                </div>
               <p class="font-black text-yellow-500 text-4xl text-center">CJP-FODR-2026</p>
            </div>
        </div>
    </div>
