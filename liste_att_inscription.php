...
if ($search) {
    $stmt = $pdo->prepare("
        ...
        WHERE e.NomEtu LIKE ? OR e.PrenomEtu LIKE ? OR a.cohorte LIKE ?
...
<head>
    <meta charset="UTF-8">
    <title>Liste des Attestations d’Inscription</title>
...
<div class="container mt-5">
    <h2 class="mb-4">Liste des Attestations d’Inscription</h2>
    ...
    <form class="mb-3" method="get" action="">
        <div class="input-group">
            <input type="text" name="search" class="form-control" placeholder="Rechercher par nom, prénom ou cohorte" value="<?= htmlspecialchars($search) ?>">
            <button type="submit" class="btn btn-primary">Rechercher</button>
            <a href="liste_inscription.php" class="btn btn-secondary">Actualiser</a>
        </div>
    </form>
        ...
        <?php if (count($req) > 0): ?>
            <?php foreach ($req as $row): ?>
                <tr>
                    ...
                    <td><?= htmlspecialchars($row['cohorte']) ?></td>
                    <td><?= htmlspecialchars($row['programme']) ?></td>
                    <td><?= htmlspecialchars($row['date_generation']) ?></td>
                    <td>
                       <a href="voir_attestation.php?id=<?= $row['id'] ?>&type=inscription" target="_blank" class="btn btn-success btn-sm">Voir</a>
